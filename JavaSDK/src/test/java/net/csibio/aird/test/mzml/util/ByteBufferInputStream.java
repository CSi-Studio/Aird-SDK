...
/**
 * A bridge between byte {@linkplain ByteBuffer buffers} and {@linkplain InputStream input
 * streams}.
 *
 * <p>
 ...
 * of a file using a {@link MappedByteBuffer}. This class can {@linkplain #map(FileChannel, MapMode)
 * expose a file of arbitrary length} as a {@linkplain RepositionableStream repositionable} {@link
 * MeasurableInputStream} that is actually based on an array of {@link MappedByteBuffer}s, each
 * mapping a <em>chunk</em> of {@link #CHUNK_SIZE} bytes.
 *
...
public class ByteBufferInputStream extends InputStream {
    ...
    /**
     * An array parallel to {@link #byteBuffer} specifying which buffers do not need to be {@linkplain
     * ByteBuffer#duplicate() duplicated} before being used.
     */
     ...
     * @param size       the sum of the {@linkplain ByteBuffer#capacity() capacities} of the byte
     *                   buffers.
     * @param curr       the current buffer (reading will start at this buffer from its current
     *                   position).
     * @param readyToUse an array parallel to <code>byteBuffer</code> specifying which buffers do not
     *                   need to be {@linkplain ByteBuffer#duplicate() duplicated} before being used
     *                   (the process will happen lazily); the array will be used internally by the
     *                   newly created byte-buffer input stream.
     */
    ...
    private long remaining() {
        return curr == n - 1 ? byteBuffer(curr).remaining() : byteBuffer(curr).remaining() + ((long) (n - 2 - curr) << CHUNK_SHIFT) + lastBufferCapacity;
    }
