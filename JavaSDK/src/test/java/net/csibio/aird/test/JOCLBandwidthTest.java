...
public class JOCLBandwidthTest {
    ...
    private static void runTest(MemoryMode memoryMode, AccessMode accessMode) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            double bandwidth = computeBandwidth(memorySizes[i], memoryMode, accessMode);
    ...
    /**
     * Compute the bandwidth in MB/s for copying a chunk of memory of the given size from the host to
     * the device with the given memory- and access mode
     *
     ...
     */
    static double computeBandwidth(int memorySize, MemoryMode memoryMode, AccessMode accessMode) {
        ...
        if (memoryMode == MemoryMode.PINNED) {
            ...
            pinnedHostData = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, memorySize, null, null);

            // Map the buffer into the host address space
            hostData = clEnqueueMapBuffer(commandQueue, pinnedHostData, CL_TRUE, CL_MAP_WRITE, 0, memorySize, 0, null, null, null);
            ...
            // pinned host buffer
            clEnqueueUnmapMemObject(commandQueue, pinnedHostData, hostData, 0, null, null);
        } else {
        ...
        deviceData = clCreateBuffer(context, CL_MEM_READ_WRITE, memorySize, null, null);
        ...
        if (accessMode == AccessMode.DIRECT) {
            if (memoryMode == MemoryMode.PINNED) {
                hostData = clEnqueueMapBuffer(commandQueue, pinnedHostData, CL_TRUE, CL_MAP_READ, 0, memorySize, 0, null, null, null);
            }

            // Copy the data from the host buffer to the
            // device a few times
            for (int i = 0; i < MEMCOPY_ITERATIONS; i++) {
                clEnqueueWriteBuffer(commandQueue, deviceData, CL_FALSE, 0, memorySize, Pointer.to(hostData), 0, null, null);
            }
            clFinish(commandQueue);
        } else {
            // Map the data from the device to the host addess space
            ByteBuffer mappedDeviceData = clEnqueueMapBuffer(commandQueue, deviceData, CL_TRUE, CL_MAP_WRITE, 0, memorySize, 0, null, null, null);
            if (memoryMode == MemoryMode.PINNED) {
                hostData = clEnqueueMapBuffer(commandQueue, pinnedHostData, CL_TRUE, CL_MAP_READ, 0, memorySize, 0, null, null, null);
            }
            // Copy the data from the host buffer to the
            ...
            clEnqueueUnmapMemObject(commandQueue, deviceData, mappedDeviceData, 0, null, null);
        }

        // Compute the bandwidth in MB/s
        long after = System.nanoTime();
        double durationS = (after - before) / 1e9;
        double bandwidthInMBs = (memorySize * MEMCOPY_ITERATIONS) / (durationS * (1 << 20));
    ...
    /**
     * Perform a default initialization by creating a context and a command queue
     */
    ...
    /**
     * Shut down and release all resources that have been allocated in {@link #initialize()}
     */
