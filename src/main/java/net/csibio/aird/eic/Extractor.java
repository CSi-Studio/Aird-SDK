...
public class Extractor {
     ...
     */
    public static float accumulation(Spectrum<float[], float[], float[]> spectrum, Float mzStart, Float mzEnd) {
     ...
     */
    public static float[][] accumulationWithGPU(List<Spectrum<float[], float[], float[]>> pairsList,
        ...
        if (delta != countInBatch) {
            for (int k = 0; k < delta; k++) {
                pairsList.add(new Spectrum<float[], float[], float[]>(new float[0], new float[0]));
            }
        }

        XIC.initialize(countInBatch);
        for (int i = 0; i < pairsList.size(); i = i + countInBatch) {
            float[] results = XIC.lowerBoundWithGPU(pairsList.subList(i, i + countInBatch), targetMzArray,
                    mzWindow);
            resMatrix[i] = results;
        }
        XIC.shutdown();
