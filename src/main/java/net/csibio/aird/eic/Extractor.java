...
public class Extractor {
   ...
   */
  public static double[][] accumulationWithGPU(List<Spectrum> pairsList, double[] targetMzArray,
      double mzWindow) {
    ...
    for (int i = 0; i < pairsList.size(); i = i + countInBatch) {
      double[] results = XIC.lowerBoundWithGPU(pairsList.subList(i, i + countInBatch),
          targetMzArray, mzWindow);
      resMatrix[i] = results;
    }
    XIC.shutdown();
    return resMatrix;
