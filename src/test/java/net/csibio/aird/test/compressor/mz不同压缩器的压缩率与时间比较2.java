...
public class mz不同压缩器的压缩率与时间比较2 {
    ...
    static HashMap<String, List<byte[]>> spectrumDeltaBytesMap = new HashMap<>();
    static HashMap<String, List<int[]>> spectrumIntsMap = new HashMap<>();
    static HashMap<String, List<int[]>> spectrumDeltaIntsMap = new HashMap<>();
...
//        AirdType.DDA, -1);
        initFile("File-DIA-Proteomics",
                "D:\\proteomics\\Project\\HYE110_6600_32Var\\HYE110_TTOF6600_32var_lgillet_I160309_001.json",
                AirdType.DIA, 12);
    }
    ...
    private static void initFile(String name, String indexPath, AirdType type, int indexNo)
        ...
        List<int[]> deltaInts = new ArrayList<>();
        List<byte[]> deltaBytes = new ArrayList<>();
        for (int i = 0; i < spectrumList.size(); i++) {
            bytesList.add(ByteTrans.doubleToByte(spectrumList.get(i).getMzs()));
            ...
            int[] deltas = Delta.delta(ints);
            deltaInts.add(deltas);
            deltaBytes.add(ByteTrans.intToByte(deltas));
            intsList.add(ints);
        }
        spectrumIntsMap.put(name, intsList);
        spectrumBytesMap.put(name, bytesList);
        spectrumDeltaIntsMap.put(name, deltaInts);
        spectrumDeltaBytesMap.put(name, deltaBytes);
    ...
    public void test_DIA() {
        spectrumListMap.forEach((key, value) -> {
            ...
            List<byte[]> deltaBytesList = spectrumDeltaBytesMap.get(key);
            List<Spectrum> spectrumList = spectrumListMap.get(key);
            List<int[]> intsList = spectrumIntsMap.get(key);
            List<int[]> deltaIntsList = spectrumDeltaIntsMap.get(key);
            ...
            test_zlib(deltaBytesList);
            test_brotli(deltaBytesList);
            test_snappy(deltaIntsList);
            test_zstd(deltaBytesList);
            test_lz4(deltaBytesList);
...
//      test_LZMA2(bytesList);
    ...
    private void test_lz4(List<byte[]> bytesList) {
        ...
        System.out.println(
                "LZ4:" + (System.currentTimeMillis() - start) + "|" + compressedSize.get() / MB + "M");
    }
    ...
    private void test_binary_pack(List<int[]> intsList) {
        ...
        System.out.println(
                "FastPFOR:" + (System.currentTimeMillis() - start) + "|" + compressedSize.get() / MB + "M");
    }
    ...
    private void test_xdvb_brotli(List<int[]> intsList) {
        ...
        AtomicLong compressedSize = new AtomicLong(0);
        intsList.parallelStream().forEach(ints -> {
            compressedSize.getAndAdd(new IntegratedXVByte().encode(ints, ByteCompType.Brotli).length);
        });
        System.out.println(
    ...
    private void test_xdvb_snappy(List<int[]> intsList) {
        ...
        AtomicLong compressedSize = new AtomicLong(0);
        intsList.parallelStream().forEach(ints -> {
            compressedSize.getAndAdd(new IntegratedXVByte().encode(ints, ByteCompType.Snappy).length);
        });
        System.out.println(
    ...
    private void test_xdvb_zstd(List<int[]> intsList) {
        ...
        AtomicLong compressedSize = new AtomicLong(0);
        intsList.parallelStream().forEach(ints -> {
            compressedSize.getAndAdd(new IntegratedXVByte().encode(ints, ByteCompType.Zstd).length);
        });
        System.out.println(
