        ...
        for (int k = 17; k < 18; k++) {
            ...
            spectrumList.parallelStream().forEach(spectrum -> {
                ...
                for (CompressorType value : byteTypes) {
                    ...
                    if (!ArrayUtil.isSame(ByteTrans.doubleToFloat(spectrum.mzs()), mzs)) {
...
//          }
                ...
                mzMap.get("ZDPD")
                        .getAndAdd(XDPD.encode(spectrum.mzs(), mzCompressor.getPrecision(), CompressorType.Zlib).length);
                mzMap.get("LDPD")
                        .getAndAdd(XDPD.encode(spectrum.mzs(), mzCompressor.getPrecision(), CompressorType.LZMA2).length);
                mzMap.get("SDPD")
                        .getAndAdd(XDPD.encode(spectrum.mzs(), mzCompressor.getPrecision(), CompressorType.Sna).length);
            });
