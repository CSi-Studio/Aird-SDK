...
public class SizeTest {
    ...
    public void testForDIA() throws Exception {
        ...
        List<ByteCompType> byteTypes = Arrays.asList(ByteCompType.Zlib, ByteCompType.Snappy, ByteCompType.Brotli);
        ...
        for (int k = 17; k < 18; k++) {
            ...
            TreeMap<Double, Spectrum> spectrumMap = diaParser.getSpectra(index);
            ...
            spectrumList.parallelStream().forEach(spectrum -> {
                ...
                byte[] mzBytes = ByteTrans.doubleToByte(spectrum.getMzs());
                byte[] intBytes = ByteTrans.doubleToByte(spectrum.getInts());
                ...
                mzMap.get("ZDPD").getAndAdd(XDPD.encode(spectrum.getMzs(), precision, ByteCompType.Zlib).length);
                mzMap.get("BDPD").getAndAdd(XDPD.encode(spectrum.getMzs(), precision, ByteCompType.Brotli).length);
                mzMap.get("SDPD").getAndAdd(XDPD.encode(spectrum.getMzs(), precision, ByteCompType.Snappy).length);
            });
